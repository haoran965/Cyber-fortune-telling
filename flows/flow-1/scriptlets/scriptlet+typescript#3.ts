...
type Inputs = Readonly<{ in: { birthType: string, sex: string, birthDay: string, desc: string, deepKey: string } }>;
...
export default async function(inputs: Inputs, context: Context) {
  console.log(inputs)
  const { birthDay, birthType, desc, sex, deepKey } = inputs.in;
  ...
  async function main() {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: `我是${birthType}${birthDay}出生的${sex}性, 分析我的${desc}, 请注意, 当前时间为${dayjs().format("YYYY-MM-DD HH:mm:ss")}` },
      ],
    });
